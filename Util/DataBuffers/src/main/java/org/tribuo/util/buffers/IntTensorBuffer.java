 ...
 */
public final class IntTensorBuffer extends TensorBuffer<IntBuffer> {
     ...
     */
    public IntTensorBuffer(IntBuffer buffer, long[] shape) {
        super(buffer, shape);
    }
     ...
     * @param shape The shape.
     */
    public IntTensorBuffer(long[] shape) {
     ...
     */
    public IntTensorBuffer(IntBuffer buffer, long[] shape, int value) {
        super(buffer, shape);
     ...
     * @param value The value.
     */
    public IntTensorBuffer(long[] shape, int value) {
        super(alloc(shape), shape);
    ...
    @Override
    public IntTensorBuffer copy() {
        IntBuffer copy = alloc(shape);
        copy.put(buffer);
        copy.rewind();
        buffer.rewind();
        return new IntTensorBuffer(copy, Arrays.copyOf(shape, shape.length));
    }
    ...
    private static IntBuffer alloc(long[] shape) {
        ...
        return ByteBuffer.allocateDirect(elements * Integer.BYTES).order(ByteOrder.nativeOrder()).asIntBuffer();
    }
}
